...
RUN true \
    && yum install -y \
        openblas-devel \
        opencv-devel \
    && provisioning/install-sw.sh mxnet apache/1.2.1 /opt/mxnet
...
ENV \
    ...
    G4REALSURFACEDATA="/opt/geant4/share/Geant4-10.4.2/data/RealSurface2.1" \
...
RUN yum install -y \
    ...
    && rpm -ihv https://github.com/atom/atom/releases/download/v1.29.0/atom.x86_64.rpm
